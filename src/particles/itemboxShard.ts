...
export class ItemShard {
	...
	constructor(scene: Scene, targ: Kart, model: nitroModel) {
		...
		this.vel = vec3.add([0, 0, 0], targ.vel, [(Math.random() - 0.5) * 5, Math.random() * 7, (Math.random() - 0.5) * 5]);
		this.dirVel = [Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5];
		this.dir = [Math.random() * 2 * Math.PI, Math.random() * 2 * Math.PI, Math.random() * 2 * Math.PI];
