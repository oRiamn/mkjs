...
export class SSEQPlayer {
	...
	playNote(thread: SSEQThread, velocity: number, duration: number, num: number): ThreadM | null {
		...
		const inst = this.bank.bank.instruments[thread.program];
		if (inst == null) return null;
		const playable = this._getInst(inst, num);
		if (playable == null) {
			// debugger;
			return null;
		}
		...
		if (thread.tie && thread.lastNote != null) {
			...
			snd = SSEQWaveCache.getWave(this.bank.waveArcs[playable.swar], playable.swav);
		...
		const targetFreq = this._noteToFreq(num + noteOffsets) / playable.freq / snd.info.mul;
		if (thread.portaKey & 0x80)
			...
			source.playbackRate.setValueAtTime(this._noteToFreq(thread.portaKey + noteOffsets) / playable.freq / snd.info.mul, baseTime);
		...
		const atk = thread.attack != null ? thread.attack : playable.attack;
		const dec = thread.decay != null ? thread.decay : playable.decay;
		const sus = thread.sustain != null ? thread.sustain : playable.sustainLvl;
		const rel = thread.release != null ? thread.release : playable.release;
