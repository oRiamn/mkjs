...
export class cameraIntro implements Camera {
	...
	private camFunc(_scene: Scene, _came: nkm_section_CAME): CamView {
		...
		return {
			p: p,
			mv: mat,
			pos: vec3.scale([0, 0, 0], vec3.transformMat4([0, 0, 0], [0, 0, 0], mat4.invert(mat4.create(), mat)), 1024),
		};
	}
	...
	getView(scene: Scene, width: number, height: number) {
		...
		if (this.curCam == null) throw new Error("Intro camera did not initialize");
