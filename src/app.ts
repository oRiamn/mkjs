...
export function getRequestAnimationFrameFnct(window: Window) {
	return (
...
export function startGameLoop(requestAnimationFrame: (callback: (time: number) => void) => number, callbacks: GameLoopCallbacks) {
	...
	function loop(timestamp: number) {
		...
		lastTime = now;
		if (frameTime > MAX_FRAME_DELTA_MS) {
			frameTime = MAX_FRAME_DELTA_MS;
		}

		timeAccumulator += frameTime;
		while (timeAccumulator >= FRAME_MS) {
			callbacks.onUpdate();
...
export function setupHUD(options?: { onChange?: () => void }) {
	...
	for (let i = 0; i < MKDSCONST.COURSES.length; i++) {
		...
		opt.value = `${i}`;
		kcls.appendChild(opt);
		if (i === MKDSCONST.CURRENTCOURSE) {
			opt.selected = true;
		}
	}
	...
	["us", "fr", "es", "ge", "it"].map((lang) => {
		...
		opt.value = lang;
		klangs.appendChild(opt);
		if (lang === MKDSCONST.CURRENTLANG) {
			opt.selected = true;
		}
	});
	...
	["CPU", "MAN"].map((ctrl) => {
		...
		opt.value = ctrl;
		kcontrols.appendChild(opt);
		if (ctrl === MKDSCONST.CONTROLTYPE) {
			opt.selected = true;
		}
	});
	...
	[3, 5].map((laptype) => {
		const opt = document.createElement("option");
		opt.textContent = `${laptype}`;
		opt.value = `${laptype}`;
		klaptype.appendChild(opt);
		if (laptype === MKDSCONST.MAX_LAP) {
			opt.selected = true;
		}
	});
}
