...
export class nitroAnimator {
	...
	private getFrames(anim: { endFrame: number; startFrame: number; speed: number }, length: number, frame: number, totalLength: number) {
	...
	private generateMatrixStack(model: nsbmd_modelData, targ: Float32Array) {
		...
		var scale = vec3.clone([model.head.scale as number, model.head.scale as number, model.head.scale as number]);
	...
	private parseRotation(rot: nsbca_rot): mat3 {
		if (rot.pivot === true) {
			var flag = rot.param;
			var pivot = mat3.clone([0, 0, 0, 0, 0, 0, 0, 0, 0]);
			var mode = flag & 15;
			var neg = (flag >> 4) & 15;
			var A = rot.a;
			var B = rot.b;
			...
			var left = horiz == 0 ? 1 : 0;
			var top = (vert == 0 ? 1 : 0) * 3;
			var right = horiz == 2 ? 1 : 2;
			var btm = (vert == 2 ? 1 : 2) * 3;
			pivot[left + top] = A;
			pivot[right + top] = B;
