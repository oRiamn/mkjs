...
export class nitroAnimator {
	...
	private getFrames(anim: { endFrame: number; startFrame: number; speed: number }, length: number, frame: number, _totalLength: number) {
		...
		const f = Math.max(0, (Math.min(frame, anim.endFrame) - anim.startFrame) * anim.speed); //speed relative time
		const end = Math.floor((anim.endFrame - anim.startFrame) * anim.speed);
		const realEnd = Math.min(length - 1, end);
	...
	setFrame(anim: number, modelind: number, frame: number): nitroAnimator_matStack {
		const b = this.bca.animData.objectData[anim];
		...
		for (let i = 0; i < b.trans.length; i++) {
			const mat = this.objMats[i];
			mat4.identity(mat);

			const a = b.trans[i]; //animated transforms
			const fa = fallback[i]; //fallback

			let translate: vec3;
			if (a.translate != null) {
				translate = [0, 0, 0];
				if (a.tlExtra[0] != null) {
					const f = this.getFrames(a.tlExtra[0], a.translate[0].length, frame, totalLength);
					const p = f[2];
					translate[0] = a.translate[0][f[0]] * (1 - p) + a.translate[0][f[1]] * p;
				} else translate[0] = a.translate[0][0];

				if (a.tlExtra[1] != null) {
					const f = this.getFrames(a.tlExtra[1], a.translate[1].length, frame, totalLength);
					const p = f[2];
					translate[1] = a.translate[1][f[0]] * (1 - p) + a.translate[1][f[1]] * p;
				} else translate[1] = a.translate[1][0];

				if (a.tlExtra[2] != null) {
					const f = this.getFrames(a.tlExtra[2], a.translate[2].length, frame, totalLength);
					const p = f[2];
			...
			if (a.rotate != null) {
				if (a.rotExtra != null) {
					const f = this.getFrames(a.rotExtra, a.rotate.length, frame, totalLength);
					const p = f[2];

					const r1 = this.parseRotation(a.rotate[f[0]]);
					const r2 = this.parseRotation(a.rotate[f[1]]);
			...
			if (a.scale != null) {
				scale = [0, 0, 0];
				if (a.scExtra[0] != null) {
					const f = this.getFrames(a.scExtra[0], a.scale[0].length, frame, totalLength);
					const p = f[2];
					scale[0] = a.scale[0][f[0]].s1 * (1 - p) + a.scale[0][f[1]].s1 * p;
				} else scale[0] = a.scale[0][0].s1;

				if (a.scExtra[1] != null) {
					const f = this.getFrames(a.scExtra[1], a.scale[1].length, frame, totalLength);
					const p = f[2];
					scale[1] = a.scale[1][f[0]].s1 * (1 - p) + a.scale[1][f[1]].s1 * p;
				} else scale[1] = a.scale[1][0].s1;

				if (a.scExtra[2] != null) {
					const f = this.getFrames(a.scExtra[2], a.scale[2].length, frame, totalLength);
					const p = f[2];
	...
	private generateMatrixStack(model: nsbmd_modelData, targ: Float32Array) {
		...
		const scale = vec3.clone([model.head.scale as number, model.head.scale as number, model.head.scale as number]);
	...
	private parseRotation(rot: nsbca_rot): mat3 {
		if (rot.pivot === true) {
			const flag = rot.param;
			const pivot = mat3.clone([0, 0, 0, 0, 0, 0, 0, 0, 0]);
			const mode = flag & 15;
			const neg = (flag >> 4) & 15;
			const A = rot.a;
			const B = rot.b;
