...
export class ObjDecor implements SceneEntityObject {
	...
	draw(view: mat4, pMatrix: mat4) {
		...
		this._res.mdl[0].draw(this._mat, pMatrix, this._animMat ?? undefined);
		if (this._forceBill) {
	...
	requireRes() {
		...
		switch (this._obji.ID) {
			...
			case 0x01a7: //monty airship
				this._forceBill = false;
				return {
					mdl: [{ nsbmd: "poo.nsbmd" }, { nsbmd: "cover.nsbmd" }, { nsbmd: "hole.nsbmd" }],
					other: [null, null, "poo.nsbtp"],
				}; //poo has nsbtp
	...
	provideRes(r: ProvidedRes) {
		...
		if (this._forceBill) {
			...
			for (let i = 0; i < models.length; i++) {
				let objs = models[i].objects.objectData;
