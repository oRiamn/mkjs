...
export class kcl {
	...
	private load(buffer: MKJSDataInput, mkwii: boolean): void {
		...
		this.planes = [undefined!]; //0 index is empty
		let minx = 0,
			maxx = 0,
			minz = 0,
			maxz = 0;
	...
	private _readBigDec(view: DataView, off: number, mkwii: boolean): number {
	...
	private _readPlane(view: DataView, offset: number): kcl_plane {
