...
export class spa implements MKJSDataFormator {
	...
	load(input: MKJSDataInput): void {
		...
		if (window.debugParticle) {
			for (let i = 0; i < particleCount; i++) {
				...
				let obj = this.particleTextures[p.textureId];
	...
	private _readParticle(view: DataView, off: number): spa_particule {
		...
		if ((flag & ParticleFlags.Bit16) != 0) {
			Bit16 = [];
			for (let i = 0; i < 20; i++) Bit16[i] = view.getUint8(off + i);
			off += 20;
		}
		...
		if ((flag & ParticleFlags.Bit25) != 0) {
			...
			Bit25 = [];
			for (let i = 0; i < 8; i++) Bit25[i] = view.getUint8(off + i);
			off += 8;
		}
		let Bit26 = undefined;
		if ((flag & ParticleFlags.Bit26) != 0) {
			Bit26 = [];
			for (let i = 0; i < 16; i++) Bit26[i] = view.getUint8(off + i);
			off += 16;
		}
		let Bit27 = undefined;
		if ((flag & ParticleFlags.Bit27) != 0) {
			Bit27 = [];
			for (let i = 0; i < 4; i++) Bit27[i] = view.getUint8(off + i);
			off += 4;
		}
		...
		if ((flag & ParticleFlags.Bit28) != 0) {
			Bit28 = [];
			for (let i = 0; i < 8; i++) Bit28[i] = view.getUint8(off + i);
			off += 8;
		}
		...
		if ((flag & ParticleFlags.Bit29) != 0) {
			Bit29 = [];
			for (let i = 0; i < 16; i++) Bit29[i] = view.getUint8(off + i);
			off += 16;
		}
	...
	private _readParticleTexture(view: DataView, off: number): spa_particuletexture {
		const stamp =
		...
		//document.body.appendChild(test);
		const glTex = null as unknown as CustomWebGLTexture;
	...
	private _premultiply(col: Uint32Array | number[]): void {
		col[0] *= col[3] / 255;
		col[1] *= col[3] / 255;
		col[2] *= col[3] / 255;
	}
