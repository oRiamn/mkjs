...
export class nsbtp implements MKJSDataFormator {
	...
	private _readAnimData(view: DataView, offset: number): nsbtp_animadata {
		...
		let nameOffset = this.matOff + this.texNamesOff;
		this.texNames = [];
		//read 16char tex names
		for (let i = 0; i < this.texTotal; i++) {
			let name = "";
			for (let j = 0; j < 16; j++) {
	...
	private _matInfoHandler(view: DataView, offset: number): nsbtp_animadata_data {
		const frames = [];
