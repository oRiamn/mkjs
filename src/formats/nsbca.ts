...
export class nsbca implements MKJSDataFormator {
	...
	private _animInfoHandler(view: DataView, off: number): nsbca_obj {
		...
		for (let i = 0; i < obj.numObj; i++) {
			const off2 = view.getUint16(offset, true) + obj.baseOff;
	...
	private _readTrans(view: DataView, off: number, obj: nsbca_obj) {
		const flag = view.getUint16(off, true); //--zyx-Sr-RZYX-T-
		off += 4;

		const transform: nsbca_obj_trans = {
			flag: undefined!,
			translate: undefined!,
			tlExtra: undefined!,
			rotate: undefined!,
		...
		if (!((flag >> 1) & 1)) {
			//T: translation
			const translate: number[][] = [[], [], []]; //store translations in x,y,z arrays
			const tlExtra = [];
			...
			for (let i = 0; i < 3; i++) {
				...
				if (f) {
					...
					const dat = view.getUint16(off + 2, true);
					inf.endFrame = dat & 0x0fff;
					inf.halfSize = (dat >> 12) & 3;
					inf.speed = this.speeds[(dat >> 14) & 3];
					inf.off = view.getUint32(off + 4, true);

					const length = Math.ceil((inf.endFrame - inf.startFrame) * inf.speed);
					const w = inf.halfSize ? 2 : 4;
		...
		if (!((flag >> 6) & 1)) {
			...
			if (f) {
				...
				inf.startFrame = view.getUint16(off, true);
				const dat = view.getUint16(off + 2, true); //low 12 bits are end frame, high 4 are size flag and speed
				inf.endFrame = dat & 0x0fff;
		...
		if (!((flag >> 9) & 1)) {
			//S: scale
			const scales: [nsbca_scale, nsbca_scale, nsbca_scale] = [[], [], []]; //store scales in x,y,z arrays
			const scExtra = [];
	...
	private _readRotation(view: DataView, off: number, obj: nsbca_obj): nsbca_rot {
		const dat = view.getInt16(off, true);
		const ind = dat & 0x7fff;
		const mode = dat >> 15;

		if (mode) {
			//rotation is pivot
			const off2 = obj.baseOff + obj.off1 + ind * 6; //jump to rotation data
			return {
				pivot: true,
			...
			const off2 = obj.baseOff + obj.off2 + ind * 10; //jump to rotation data
			const d1 = view.getUint16(off2, true);
			const d2 = view.getUint16(off2 + 2, true);
			const d3 = view.getUint16(off2 + 4, true);
			const d4 = view.getUint16(off2 + 6, true);
			const d5 = view.getUint16(off2 + 8, true);

			const i6 = ((d5 & 7) << 12) | ((d1 & 7) << 9) | ((d2 & 7) << 6) | ((d3 & 7) << 3) | (d4 & 7);
			//if (i6&4096) i6 = (-8192)+i6;

			const v1: vec3 = [d1 >> 3, d2 >> 3, d3 >> 3];
			const v2: vec3 = [d4 >> 3, d5 >> 3, i6];
			...
			const v3 = vec3.cross([0, 0, 0], v1, v2);

			const mat = mat3.clone([v1[0], v1[1], v1[2], v2[0], v2[1], v2[2], v3[0], v3[1], v3[2]]);
