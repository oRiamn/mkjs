...
export class nclr implements MKJSDataFormator {
	...
	private _loadPLTT(view: DataView): NclrPaletteObj {
		...
		let colsPerPal = bitDepth == 4 ? 256 : 16;
		let realPalCount = (blockSize - 0x18) / 2;

		offset += 0x18;
		const palettes: NclrPalette[] = [];
		let curPal: NclrPaletteColor[] = [];
		for (let i = 0; i < realPalCount; i++) {
	...
	private _loadPCMP(view: DataView): NclrPaletteCountMap {
		//palette count map, supposedly. maps each palette to an ID
		let offset = this.sectionOffsets[1] - 8;
	...
	private _readPalColour(view: DataView, ind: number): NclrPaletteColor {
		let col = view.getUint16(ind, true);
		let f = 255 / 31;
