...
export class ncer implements MKJSDataFormator {
	...
	private _getSize(shape: number, size: number, double: boolean): [number, number] {
		let dim = this.dimensions[size][shape];
		if (double) return [dim[0] * 2, dim[1] * 2];
		return [dim[0], dim[1]];
	}

	private _readOAM(view: DataView, offset: number): NcerCebkImageCell {
		//see ds docs. really, any of them?
		let obj0 = view.getUint16(offset, true);
		let obj1 = view.getUint16(offset + 2, true);
		let obj2 = view.getUint16(offset + 4, true);

		let rsFlag = (obj0 & 0x100) > 0;
		let x = obj1 & 0x01ff;
		let y = obj0 & 0xff;
	...
	private _loadCEBK(view: DataView) {
		//cell bank
		...
		offset += 0x20;
		let tableEnd = offset + imageCount * (8 + bankType * 8);
		for (let i = 0; i < imageCount; i++) {
			...
			let offset2 = tableEnd + imageoffset;
			const cells = [];
			for (let j = 0; j < numCells; j++) {
				let cell = this._readOAM(view, offset2);
				offset2 += 6;
				cells.push(cell);
			}
			images.push({
				...
				yMin,
				cells,
				partitionOffset: undefined as unknown as number,
				partitionSize: undefined as unknown as number,
			});
		}
		...
		if (partitionDataOffset != 0) {
			//not sure what this does, just that it's here
			let pOff = this.sectionOffsets[0] + partitionDataOffset;
